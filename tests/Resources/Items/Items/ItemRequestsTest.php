...
it('can get Items', function () {
    try {
        ...
        Assert::markTestSkipped('Items endpoint unavailable: '.$e->getMessage());
    }

    if (count($items) === 0) {
        Assert::markTestSkipped('No items available');
    }
...
it('can search Items', function () {
    ...
    if (! $searchValue) {
        Assert::markTestSkipped('No searchable item name available');
    }
