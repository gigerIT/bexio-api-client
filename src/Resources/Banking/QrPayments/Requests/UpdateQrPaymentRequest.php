...
class UpdateQrPaymentRequest extends Request implements HasBody
{
    use HasJsonBody;
    ...
    public function __construct(protected readonly QrPayment $qrPayment)
    {
        if ($this->qrPayment->bank_account_id === null) {
            throw new \InvalidArgumentException('bank_account_id is required to update a QR payment.');
        }

        if ($this->qrPayment->id === null) {
            throw new \InvalidArgumentException('id is required to update a QR payment.');
        }
    }

    public function resolveEndpoint(): string
    {
        return "/3.0/banking/bank_accounts/{$this->qrPayment->bank_account_id}/qr_payments/{$this->qrPayment->id}";
    }
